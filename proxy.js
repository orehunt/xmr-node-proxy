...
function slaveMessageHandler(message) {
  switch (message.type) {
    case "newBlockTemplate":
      if (message.host in activePools) {
        if (activePools[message.host].activeBlocktemplate) {
          debug.workers(
            `Received a new block template for ${message.host} and have one in cache.  Storing`
          );
          activePools[message.host].pastBlockTemplates.enq(
            activePools[message.host].activeBlocktemplate
          );
        } else {
          debug.workers(
            `Received a new block template for ${message.host} do not have one in cache.`
          );
        }
        activePools[message.host].activeBlocktemplate = new activePools[
          message.host
        ].coinFuncs.BlockTemplate(message.data);
    ...
    case "changePool":
      if (
        activeMiners.hasOwnProperty(message.worker) &&
        activePools.hasOwnProperty(message.pool)
      ) {
...
function readConfig() {
  ...
  for (let key in local_conf) {
    if (
      local_conf.hasOwnProperty(key) &&
      (typeof global.config[key] === "undefined" ||
...
function Pool(poolData) {
  ...
  this.devPool =
    poolData.hasOwnProperty("devPool") && poolData.devPool === true;
  ...
  this.algos_perf = this.default_algos_perf =
    poolData.algo_perf && poolData.algo_perf instanceof Object
      ? poolData.algo_perf
      : DEFAULT_ALGO_PERF;
  const algo_arr = poolData.algo
    ? poolData.algo instanceof Array
      ? poolData.algo
      : [poolData.algo]
    : Object.keys(this.algos_perf);
  this.default_algo_set = {};
  this.algos = {};

  for (let i in algo_arr)
    this.algos[algo_arr[i]] = this.default_algo_set[algo_arr[i]] = 1;
  this.blob_type = poolData.blob_type;

  setInterval(
    function (pool) {
      if (pool.keepAlive && pool.socket && is_active_pool(pool.hostname))
  ...
  this.close_socket = function () {
    try {
      ...
      console.warn(
        global.threadName + "Had issues murdering the old socket. Om nom: " + e
      );
    }
    this.socket = null;
  };
  ...
  this.connect = function (hostname) {
    function connect2(pool) {
      ...
      if (pool.ssl) {
        pool.socket = tls
          .connect(pool.port, pool.hostname, {
            rejectUnauthorized: pool.allowSelfSignedSSL,
          })
          .on("connect", () => {
          ...
          .on("error", (err) => {
            ...
            console.warn(
              `${global.threadName}SSL pool socket connect error from ${pool.hostname}: ${err}`
            );
          });
      } else {
        pool.socket = net
          ...
          .on("error", (err) => {
            setTimeout(connect2, 30 * 1000, pool);
            console.warn(
              `${global.threadName}Plain pool socket connect error from ${pool.hostname}: ${err}`
            );
          });
      }
    }
  ...
  this.update_algo_perf = function (algos, algos_perf) {
    ...
    if (algos_str === prev_algos_str && algos_perf_str === prev_algos_perf_str)
      return;
    const curr_time = Date.now();
    if (
      !this.last_common_algo_notify_time ||
      curr_time - this.last_common_algo_notify_time > 5 * 60 * 1000 ||
      algos_str !== prev_algos_str
    ) {
      console.log(
        "Setting common algo: " +
          algos_str +
...
function balanceWorkers() {
  ...
  for (let poolName in activePools) {
    if (activePools.hasOwnProperty(poolName)) {
      ...
      if (pool.devPool) {
        ...
        console.error(
          `${global.threadName}Pool ${poolName} is disabled due to issues with it`
        );
      }
  ...
  for (let coin in poolStates) {
    if (poolStates.hasOwnProperty(coin)) {
      if (poolStates[coin].totalPercentage !== 100) {
        debug.balancer(
          `Pools on ${coin} are using ${poolStates[coin].totalPercentage}% balance.  Adjusting.`
        );
        ...
        if (poolStates[coin].totalPercentage) {
          ...
          for (let pool in poolStates[coin]) {
            if (
              poolStates[coin].hasOwnProperty(pool) &&
              activePools.hasOwnProperty(pool)
            ) {
              if (poolStates[coin][pool].devPool || !is_active_pool(pool))
                continue;
          ...
          debug.balancer(
            `No active pools for ${coin} coin, so waiting for the next cycle.`
          );
          continue;
        }
      }
      ...
      for (let minerID in activeWorkers[workerID]) {
        if (activeWorkers[workerID].hasOwnProperty(minerID)) {
          ...
          try {
            ...
            poolStates[minerCoin][miner.pool].miners[`${workerID}_${minerID}`] =
              miner.avgSpeed;
          } catch (err) {}
        }
      }
  ...
  for (let coin in poolStates) {
    if (poolStates.hasOwnProperty(coin) && minerStates.hasOwnProperty(coin)) {
      ...
      if (devPool) {
        let devHashrate = Math.floor(
          coinMiners.hashrate * (global.config.developerShare / 100)
        );
        coinMiners.hashrate -= devHashrate;
        coinPools[devPool].idealRate = devHashrate;
        debug.balancer(
          `DevPool on ${coin} is enabled.  Set to ${global.config.developerShare}% and ideally would have ${coinPools[devPool].idealRate}.  Currently has ${coinPools[devPool].hashrate}`
        );
        if (
          is_active_pool(devPool) &&
          coinPools[devPool].idealRate > coinPools[devPool].hashrate
        ) {
          lowPools[devPool] =
            coinPools[devPool].idealRate - coinPools[devPool].hashrate;
          debug.balancer(
            `Pool ${devPool} is running a low hashrate compared to ideal.  Want to increase by: ${lowPools[devPool]} h/s`
          );
        } else if (
          !is_active_pool(devPool) ||
          coinPools[devPool].idealRate < coinPools[devPool].hashrate
          ...
          highPools[devPool] =
            coinPools[devPool].hashrate - coinPools[devPool].idealRate;
          debug.balancer(
            `Pool ${devPool} is running a high hashrate compared to ideal.  Want to decrease by: ${highPools[devPool]} h/s`
          );
        }
      }
      for (let pool in coinPools) {
        if (
          coinPools.hasOwnProperty(pool) &&
          pool !== devPool &&
          activePools.hasOwnProperty(pool)
        ) {
          coinPools[pool].idealRate = Math.floor(
            coinMiners.hashrate * (coinPools[pool].percentage / 100)
          );
          if (
            is_active_pool(pool) &&
            coinPools[pool].idealRate > coinPools[pool].hashrate
          ) {
            lowPools[pool] =
              coinPools[pool].idealRate - coinPools[pool].hashrate;
            debug.balancer(
              `Pool ${pool} is running a low hashrate compared to ideal.  Want to increase by: ${lowPools[pool]} h/s`
            );
          } else if (
            !is_active_pool(pool) ||
            coinPools[pool].idealRate < coinPools[pool].hashrate
            ...
            highPools[pool] =
              coinPools[pool].hashrate - coinPools[pool].idealRate;
            debug.balancer(
              `Pool ${pool} is running a high hashrate compared to ideal.  Want to decrease by: ${highPools[pool]} h/s`
            );
          }
      ...
      if (
        Object.keys(highPools).length === 0 &&
        Object.keys(lowPools).length === 0
      ) {
        debug.balancer(
          `No high or low ${coin} coin pools, so waiting for the next cycle.`
        );
        continue;
      }
      let freed_miners = {};
          ...
          if (highPools.hasOwnProperty(pool)) {
            for (let miner in coinPools[pool].miners) {
              if (coinPools[pool].miners.hasOwnProperty(miner)) {
                if (
                  (!is_active_pool(pool) ||
                    coinPools[pool].miners[miner] <= highPools[pool]) &&
                  coinPools[pool].miners[miner] !== 0
                ) {
                  highPools[pool] -= coinPools[pool].miners[miner];
                  freed_miners[miner] = coinPools[pool].miners[miner];
                  debug.balancer(
                    `Freeing up ${miner} on ${pool} for ${freed_miners[miner]} h/s`
                  );
          ...
          if (lowPools.hasOwnProperty(pool)) {
              ...
              for (let miner in freed_miners) {
                if (freed_miners.hasOwnProperty(miner)) {
                  if (freed_miners[miner] <= lowPools[pool]) {
                    ...
                    debug.balancer(
                      `Snagging up ${miner} for ${pool} for ${freed_miners[miner]} h/s`
                    );
            ...
            if (lowPools[pool] > 100) {
              for (let donatorPool in coinPools) {
                if (
                  coinPools.hasOwnProperty(donatorPool) &&
                  !lowPools.hasOwnProperty(donatorPool)
                ) {
                  for (let miner in coinPools[donatorPool].miners) {
                    if (coinPools[donatorPool].miners.hasOwnProperty(miner)) {
                      if (
                        coinPools[donatorPool].miners[miner] <=
                          lowPools[pool] &&
                        coinPools[donatorPool].miners[miner] !== 0
                      ) {
                        minerChanges[pool].push(miner);
                        lowPools[pool] -= coinPools[donatorPool].miners[miner];
                        debug.balancer(
                          `Moving ${miner} for ${pool} from ${donatorPool} for ${coinPools[donatorPool].miners[miner]} h/s`
                        );
            ...
            if (lowPools.hasOwnProperty(pool) && pool !== devPool) {
              ...
              for (let miner in freed_miners) {
                if (freed_miners.hasOwnProperty(miner)) {
                  ...
                  lowPools[pool] -= freed_miners[miner];
                  debug.balancer(
                    `Moving overflow ${miner} for ${pool} for ${freed_miners[miner]} h/s`
                  );
                  delete freed_miners[miner];
                }
              }
      ...
      for (let pool in minerChanges) {
        if (
          minerChanges.hasOwnProperty(pool) &&
          minerChanges[pool].length > 0
        ) {
          minerChanges[pool].forEach(function (miner) {
            let minerBits = miner.split("_");
            if (cluster.workers[minerBits[0]])
...
function enumerateWorkerStats() {
  let stats,
    global_stats = { miners: 0, hashes: 0, hashRate: 0, diff: 0 };
  let pool_algos = {};
  let pool_algos_perf = {};
  for (let poolID in activeWorkers) {
    if (activeWorkers.hasOwnProperty(poolID)) {
      ...
      let inactivityDeadline =
        typeof global.config.minerInactivityTime === "undefined"
          ? Math.floor(Date.now() / 1000) - 120
          : global.config.minerInactivityTime <= 0
          ? 0
          : Math.floor(Date.now() / 1000) - global.config.minerInactivityTime;
        ...
        if (activeWorkers[poolID].hasOwnProperty(workerID)) {
          ...
          if (typeof workerData !== "undefined") {
            try {
              ...
              if (!miner_algos)
                miner_algos = activePools[workerData.pool].default_algo_set;
              if (workerData.pool in pool_algos) {
                // compute union of miner_algos and pool_algos[workerData.pool]
                for (let algo in pool_algos[workerData.pool]) {
                  if (!(algo in miner_algos))
                    delete pool_algos[workerData.pool][algo];
                }
              } else {
              ...
              if (workerData.algos_perf) {
                // only process smart miners and add algo_perf from all smart miners
                for (let algo in workerData.algos_perf) {
                  if (algo in pool_algos_perf[workerData.pool])
                    pool_algos_perf[workerData.pool][algo] +=
                      workerData.algos_perf[algo];
                  else
                    pool_algos_perf[workerData.pool][algo] =
                      workerData.algos_perf[algo];
                }
      ...
      debug.workers(
        `Worker: ${poolID} currently has ${stats.miners} miners connected at ${
          stats.hashRate
        } h/s with an average diff of ${Math.floor(stats.diff / stats.miners)}`
      );
    }
  }
  ...
  for (let coin in poolStates) {
    ...
    for (let pool in poolStates[coin]) {
      if (
        !poolStates[coin].hasOwnProperty(pool) ||
        !activePools.hasOwnProperty(pool) ||
        poolStates[coin][pool].devPool ||
        poolStates[coin][pool].hashrate === 0
      )
        continue;
      if (pool_hs != "") pool_hs += ", ";
  ...
  for (let pool in pool_algos) {
    ...
    if (Object.keys(pool_algos_perf2).length === 0)
      pool_algos_perf2 = activePools[pool].default_algos_perf;
    activePools[pool].update_algo_perf(pool_algos[pool], pool_algos_perf2);
    if (Object.keys(pool_algos[pool]).length == 1) {
      if ("c29s" in pool_algos[pool])
        hs_algo = hs_algo === "c29s" || hs_algo === "" ? "c29s" : "h/s";
      else if ("c29v" in pool_algos[pool])
        hs_algo = hs_algo === "c29v" || hs_algo === "" ? "c29v" : "h/s";
      else hs_algo = "h/s";
    } else {
      hs_algo = "h/s";
    }
  ...
  console.log(
    `The proxy currently has ${global_stats.miners} miners connected at ${hs}${pool_hs}` +
      (global_stats.miners
        ? ` with an average diff of ${Math.floor(
            global_stats.diff / global_stats.miners
          )}`
        : "")
  );
}
...
function poolSocket(hostname) {
  ...
  socket
      ...
      if (dataBuffer.indexOf("\n") !== -1) {
        ...
        for (let i = 0; i < messages.length; i++) {
          ...
          try {
            ...
            console.warn(
              `${global.threadName}Pool wrong reply error from ${pool.hostname}: ${message}`
            );
            socket.destroy();
    ...
    .on("error", (err) => {
      console.warn(
        `${global.threadName}Pool socket error from ${pool.hostname}: ${err}`
      );
    ...
    .on("close", () => {
      console.warn(
        `${global.threadName}Pool socket closed from ${pool.hostname}`
      );
      activePools[pool.hostname].disable();
      setTimeout(activePools[pool.hostname].connect, 30 * 1000, pool.hostname);
    });
...
function handlePoolMessage(jsonData, hostname) {
  ...
  if (jsonData.hasOwnProperty("method")) {
    ...
    if (jsonData.error !== null) {
      console.error(
        `${global.threadName}Error response from pool ${
          pool.hostname
        }: ${JSON.stringify(jsonData.error)}`
      );
      if (
        jsonData.error instanceof Object &&
        typeof jsonData.error.message === "string" &&
        jsonData.error.message.includes("Unauthenticated")
      )
...
function handleNewBlockTemplate(blockTemplate, hostname) {
  ...
  console.log(
    `Received new block template on ${blockTemplate.height} height${algo_variant} with ${blockTemplate.target_diff} target difficulty from ${pool.hostname}`
  );
  ...
  if (!blockTemplate.algo)
    blockTemplate.algo = pool.coinFuncs.detectAlgo(
      pool.default_algo_set,
      16 * parseInt(blockTemplate.blocktemplate_blob[0]) +
        parseInt(blockTemplate.blocktemplate_blob[1])
    );
  if (!blockTemplate.blob_type) blockTemplate.blob_type = pool.blob_type;
  pool.activeBlocktemplate = new pool.coinFuncs.MasterBlockTemplate(
    blockTemplate
  );
...
function is_active_pool(hostname) {
  ...
  for (let poolName in activePools) {
    ...
    if (
      (cluster.isMaster && !pool2.socket) ||
      !pool2.active ||
      pool2.activeBlocktemplate === null
    )
      continue;
    if (
      Math.abs(
        pool2.activeBlocktemplate.height - pool.activeBlocktemplate.height
      ) > 1000
    )
      continue; // different coin templates, can't compare here
    if (pool2.activeBlocktemplate.height > top_height)
      top_height = pool2.activeBlocktemplate.height;
  }
...
function match_miner_algo_to_pool(miner, portData) {
  if (
    !defaultPools.hasOwnProperty(portData.coin) ||
    !is_active_pool(defaultPools[portData.coin])
  ) {
    for (let poolName in activePools) {
      if (activePools.hasOwnProperty(poolName)) {
        let pool = activePools[poolName];
    ...
    if (p) {
      ...
      if (bT && bT.blob) {
        ...
        if (!(pool_algo in miner.algos)) {
          ...
          for (let poolName in activePools) {
            if (poolName == miner.pool || activePools[poolName].devPool)
              continue;
            if (poolName) {
...
function Miner(id, params, ip, pushMessage, portData, minerSocket) {
  ...
  if (params.algo && params.algo instanceof Array) {
    // To report union of defined algo set to the pool for all its miners
  ...
  this.minerName =
    this.identifier && this.identifier != "x"
      ? this.identifier + " (" + this.ip + ")"
      : this.ip;
  ...
  this.getNewJob = function (bashCache) {
    return this.coinFuncs.getJob(
      this,
      activePools[this.pool].activeBlocktemplate,
      bashCache
    );
  };
  ...
  this.minerStats = function () {
    ...
    return {
      ...
      hashes: this.hashes,
      avgSpeed:
        this.hashes / Math.floor((Date.now() - this.connectTime) / 1000),
  ...
  this.updateDifficulty = function () {
    if (this.hashes > 0 && !this.fixed_diff) {
      const new_diff =
        Math.floor(
          this.hashes / Math.floor((Date.now() - this.connectTime) / 1000)
        ) * this.coinSettings.shareTargetTime;
...
function isAllowedLogin(username, password) {
  ...
  if (
    typeof global.config["accessControl"] !== "object" ||
  ...
  // If user is not in the list ...
    ...
    if (
      lastAccessControlLoadTime === null ||
      ...
      accessControl = JSON.parse(
        fs.readFileSync(global.config["accessControl"]["controlFile"])
      );
...
function isInAccessControl(username, password) {
  return (
    typeof accessControl[username] !== "undefined" &&
    accessControl[username] === password
  );
}

function handleMinerData(
  minerSocket,
  id,
  method,
  params,
  ...
  switch (method) {
      ...
      if (global.config.keepOfflineMiners) {
        for (let miner_id in activeMiners) {
          if (activeMiners.hasOwnProperty(miner_id)) {
            ...
            if (
              realMiner.socket.destroyed &&
              realMiner.identifier === miner.identifier &&
              realMiner.ip === miner.ip &&
              realMiner.agent === miner.agent
            ) {
    ...
    case "submit": {
      ...
      if (typeof params.job_id === "number")
        params.job_id = params.job_id.toString(); // for grin miner
      ...
      const is_bad_nonce = miner.coinFuncs.blobTypeGrin(blob_type_num)
        ? typeof params.nonce !== "number" ||
          !(params.pow instanceof Array) ||
          params.pow.length != miner.coinFuncs.c29ProofSize(blob_type_num)
        : typeof params.nonce !== "string" ||
          !(miner.coinFuncs.nonceSize(blob_type_num) == 8
            ? nonceCheck64.test(params.nonce)
            : nonceCheck32.test(params.nonce));
      ...
      const nonce_test = miner.coinFuncs.blobTypeGrin(blob_type_num)
        ? params.pow.join(":")
        : params.nonce;
      ...
      let blockTemplate =
        activeBlockTemplate.id === job.templateID
          ? activeBlockTemplate
      ...
      let shareAccepted = miner.coinFuncs.processShare(
        miner,
        job,
        blockTemplate,
        params
...
function activateHTTP() {
  var jsonServer = http.createServer((req, res) => {
    if (global.config.httpUser && global.config.httpPass) {
      ...
      if (
        username !== global.config.httpUser ||
        password !== global.config.httpPass
      ) {
        res.statusCode = 401;
        res.setHeader("WWW-Authenticate", 'Basic realm="Secure Area"');
    ...
    if (req.url == "/") {
      ...
      for (let miner_id in miners) {
        ...
        const name = miner.minerName;
        let avgSpeed = miner.active
          ? support.human_hashrate(
              miner.avgSpeed,
              activePools[miner.pool].activeBlocktemplate.algo
            )
          : "offline";
        ...
        tableBody += `
				<tr>
					...
					<td><TAB TO=t10><div class="tooltip">${
            agent_parts[0]
          }<span class="tooltiptext">${miner.agent}</div></td>
				</tr>
				`;
      }
      for (let poolName in poolHashrate) {
        let poolPercentage = totalHashrate
          ? ((100 * poolHashrate[poolName]) / totalHashrate).toFixed(2)
          : "100.0";
        let targetDiff = activePools[poolName].activeBlocktemplate
          ? activePools[poolName].activeBlocktemplate.targetDiff
          : "?";
        let walletId = activePools[poolName].username;
        const hs = support.human_hashrate(
          poolHashrate[poolName],
          activePools[poolName].activeBlocktemplate.algo
        );
        if (poolName.includes("moneroocean")) {
          let algo_variant = "";
          if (activePools[poolName].activeBlocktemplate.algo)
            algo_variant +=
              "algo: " + activePools[poolName].activeBlocktemplate.algo;
          if (activePools[poolName].activeBlocktemplate.variant) {
            if (algo_variant != "") algo_variant += ", ";
            algo_variant +=
              "variant: " + activePools[poolName].activeBlocktemplate.variant;
          }
          if (algo_variant != "") algo_variant = " (" + algo_variant + ")";
          const hs = support.human_hashrate(
            poolHashrate[poolName],
...
`);
  ...
  jsonServer.listen(
    global.config.httpPort || "8081",
    global.config.httpAddress || "localhost"
  );
}
  ...
  async.each(global.config.listeningPorts, function (portData) {
    ...
    let handleMessage = function (socket, jsonData, pushMessage) {
      if (!jsonData.id) {
        ...
        console.warn(
          global.threadName + "Miner RPC request missing RPC method"
        );
        return;
      }
    ...
    function socketConn(socket) {
      ...
      let pushMessage = function (body) {
        ...
        body.jsonrpc = "2.0";
        debug.miners(
          `Data sent to miner (pushMessage): ` + JSON.stringify(body)
        );
    ...
    if ("ssl" in portData && portData.ssl === true) {
      ...
      server.listen(portData.port, global.config.bindAddress, function (error) {
        ...
        console.log(
          global.threadName + "Started server on port: " + portData.port
        );
      });
      server.on("error", function (error) {
        console.error(
          global.threadName +
            "Can't bind server to " +
            portData.port +
            " SSL port!"
        );
      });
      ...
      server.on("error", function (error) {
        console.error(
          global.threadName + "Can't bind server to " + portData.port + " port!"
        );
      });
    }
  });
  ...
  for (let badPool in activePools) {
    if (activePools.hasOwnProperty(badPool) && !activePools[badPool].active) {
      for (let pool in activePools) {
        if (
          activePools.hasOwnProperty(pool) &&
          !activePools[pool].devPool &&
          activePools[pool].coin === activePools[badPool].coin &&
          activePools[pool].active
        ) {
          for (let miner in activeMiners) {
            if (activeMiners.hasOwnProperty(miner)) {
              let realMiner = activeMiners[miner];
...
if (cluster.isMaster) {
  ...
  try {
    ...
    console.error(
      `${global.threadName}Unable to set the number of workers via arguments.  Make sure to run npm install!`
    );
    numWorkers = require("os").cpus().length;
  }
  ...
  global.config.pools.forEach(function (poolData) {
    ...
    if (
      !activePools.hasOwnProperty(
        activePools[poolData.hostname].coinFuncs.devPool.hostname
      )
    ) {
      activePools[
        activePools[poolData.hostname].coinFuncs.devPool.hostname
      ] = new Pool(activePools[poolData.hostname].coinFuncs.devPool);
    }
  });
  process.send({ type: "needPoolState" });
  ...
  setInterval(function () {
    for (let minerID in activeMiners) {
      if (activeMiners.hasOwnProperty(minerID)) {
        process.send({
          minerID: minerID,
          data: activeMiners[minerID].minerStats(),
          type: "workerStats",
        });
      }
    }
